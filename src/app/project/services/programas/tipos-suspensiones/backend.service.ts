...
export class BackendTiposSuspensionesService {
    ...
    async getSuspensiones(namesCrud: NamesCrud) {
        try {
    ...
    async insertSuspension(params: any, namesCrud: NamesCrud) {
        try {
            const response = await this.serviceUtils.checkResponse(
                await this.invoker.httpInvoke(
                    this.serviceUtils.generateServiceMongo('suspensiones/insertSuspension'),
                    params
                ),
                namesCrud
            );
    ...
    async updateSuspension(params: any, namesCrud: NamesCrud) {
        try {
            const response = await this.serviceUtils.checkResponse(
                await this.invoker.httpInvoke(
                    this.serviceUtils.generateServiceMongo('suspensiones/updateSuspension'),
                    params
                ),
                namesCrud
            );
    ...
    async deleteSuspension(params: any, namesCrud: NamesCrud) {
        try {
            return await this.invoker.httpInvoke('suspensiones/deleteSuspension',{suspensionToDelete: params});
        } catch (error: any) {
            this.errorTemplateHandler.processError(
                error, 
                {
