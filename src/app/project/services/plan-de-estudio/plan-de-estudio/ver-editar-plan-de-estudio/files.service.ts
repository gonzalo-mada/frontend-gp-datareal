...
export class FilesVerEditarPlanEstudioService {
    ...
    initFiles(){
        this.subscription.add(this.uploaderFilesService.contextUpdate$.subscribe( context => {
            if (context && context.component.name === 'ver/editar-plandeestudio') {
        ...
        this.subscription.add(this.uploaderFilesService.validatorFiles$.subscribe( async from => {
            if (from && from.context.component.name === 'ver/editar-plandeestudio'){
            ...
            if (from) {
                if (from.mode === 'd') {
                    if (from.context.component.collection) {
                        console.log("DESCARGA REALIZADA DESDE SERVICIO FILES VER/EDITAR PLAN DE ESTUDIO");
    ...
    async loadDocsWithBinary(cod_plan_estudio: number, from: string){
        this.uploaderFilesService.setLoading(true,true);  
        const files = await this.backend.getDocsMongo(cod_plan_estudio, from);
