...
export class FilesMencionesService {
    ...
    initFiles(){
        this.subscription.add(this.uploaderFilesService.contextUpdate$.subscribe( context => {
            if (context && context.component.name === 'menciones') {
                // console.log("-->context files.service mencion",context);
                this.setFiles();

            }
        }));
        this.subscription.add(this.uploaderFilesService.validatorFiles$.subscribe( async from => {
        if (from && from.context.component.name === 'menciones'){
            // console.log("valitador files.service mencione",from);
        ...
        this.subscription.add(this.uploaderFilesService.downloadDoc$.subscribe(from => {
            if (from) {
                if (from.context.component.name === 'menciones') {
                    console.log("DESCARGA REALIZADA DESDE SERVICIO FILES MENCIONES");
