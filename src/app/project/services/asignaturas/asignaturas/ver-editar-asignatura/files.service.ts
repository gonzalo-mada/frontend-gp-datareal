...
export class FilesVerEditarAsignatura {
    ...
    initFiles(){
        this.subscription.add(this.uploaderFilesService.contextUpdate$.subscribe( context => {
            if (context && context.component.name === 'ver/editar-asignatura') {
        ...
        this.subscription.add(this.uploaderFilesService.validatorFiles$.subscribe( async from => {
            if (from && from.context.component.name === 'ver/editar-asignatura'){
            ...
            if (from) {
                if (from.mode === 'd') {
                    if (from.context.component.collection) {
                        console.log("DESCARGA REALIZADA DESDE SERVICIO FILES VER/EDITAR ASIGNATURA");
    ...
    async loadDocsWithBinary(cod_asignatura: string, from: string){
        this.uploaderFilesService.setLoading(true,true);  
        const files = await this.backend.getDocsMongo(cod_asignatura, from);
