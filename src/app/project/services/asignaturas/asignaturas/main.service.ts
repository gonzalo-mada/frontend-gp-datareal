...
export class AsignaturasMainService {
  ...
  async getAsignaturasToOverlay(mode: 'pre_req' | 'secuenciales' | 'paralelas' | string, cod_asignatura: string): Promise<boolean>{
    try {
      this.op_asign = [];
      ...
      switch (mode) {
        case 'pre_req':
          ...
          response = response.flatMap((item: any) => item.prerrequisitos);
          this.op_asign = [...response];
        break;

        case 'secuenciales':
          response = await this.backend.getAsignsSecuencialesParalelasPorAsignatura(params);
          response = response.flatMap((item: any) => item.secuenciales);
          this.op_asign = [...response];
        break;

        case 'paralelas':
          response = await this.backend.getAsignsSecuencialesParalelasPorAsignatura(params);
          response = response.flatMap((item: any) => item.paralelas);
          this.op_asign = [...response];
        break;
