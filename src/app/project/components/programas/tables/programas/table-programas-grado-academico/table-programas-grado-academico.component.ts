...
export class TableProgramasGradoAcademicoComponent implements OnInit, OnDestroy {
  ...
  async onRowExpand(event: TableRowExpandEvent) {
    try {
      ...
      this.uploaderFilesService.setContext('show','programa','ver-programa','Grado académico');
      const files = await this.programasService.getDocumentosWithBinary(parseInt(cod_programa),'grado_academico');
