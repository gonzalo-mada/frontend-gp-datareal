...
@Component({
  selector: 'app-paralela-secuencial',
  templateUrl: './paralela-secuencial.component.html',
  styles: [
  ]
})
export class ParalelaSecuencialComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.main.setOrigen('paralela_secuencial');
  ...
  ngOnDestroy(): void {
	...
	async changePlanDeEstudio(event:any){
		...
		await this.main.getAsignaturasSecuencialesParalelasGroupedNotForm();
	}
